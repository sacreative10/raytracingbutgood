...
{
public:
    dielectric(float idx_of_refraction) : m_idx_of_refraction(idx_of_refraction){}

    virtual bool scatter(const Ray & r_in, const hitrecord & rec, color & attenuation, Ray & scattered) const override
    {
        attenuation = color(1.0, 1.0, 1.0);
        float refraction_ratio = rec.front_face ? (1.0f / m_idx_of_refraction) : m_idx_of_refraction;
        ...
        if(cannot_refract || SchlickApprox(cos_theta, refraction_ratio) > random_float())
